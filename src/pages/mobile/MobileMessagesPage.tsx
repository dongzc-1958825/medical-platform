...
const MESSAGE_TYPES = {
  drug: { icon: "💊", label: "新药信息", count: 12, color: "bg-blue-50 text-blue-600" },
  article: { icon: "📚", label: "专业文章", count: 8, color: "bg-purple-50 text-purple-600" },
  announcement: { icon: "📢", label: "公告发布", count: 5, color: "bg-green-50 text-green-600" },
  effect: { icon: "🌟", label: "特效分享", count: 15, color: "bg-yellow-50 text-yellow-600" },
  warning: { icon: "⚠️", label: "前车之鉴", count: 7, color: "bg-red-50 text-red-600" }
};
...
const MobileMessagesPage: React.FC = () => {
  ...
  const [selectedType, setSelectedType] = useState("drug"); // 默认选中新药信息
  ...
  const filteredMessages = mockMessages.filter(msg => msg.type === selectedType);
