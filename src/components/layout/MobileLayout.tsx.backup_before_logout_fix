...
import { Outlet, NavLink, useLocation } from 'react-router-dom'; // 添加useLocation
...
const MobileLayout = () => {
  ...
  const handleLogout = async () => {
    console.log('🚪 MobileLayout: 开始退出');
    console.log('👤 当前用户:', user?.email);
    
    try {
      await logout();
      console.log('✅ MobileLayout: logout()完成');
      ...
      console.error('❌ MobileLayout: 退出失败:', error);
      window.location.reload();
    }
  };
