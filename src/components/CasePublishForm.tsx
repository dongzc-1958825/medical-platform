...
const CasePublishForm: React.FC<CasePublishFormProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.title.trim()) newErrors.title = "病例标题不能为空";
    if (!formData.patientName.trim()) newErrors.patientName = "患者姓名不能为空";
    if (!formData.diagnosis.trim()) newErrors.diagnosis = "诊断结果不能为空";
    if (formData.symptoms.length === 0) newErrors.symptoms = "请至少添加一个症状";
    setErrors(newErrors);
      ...
      >
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* 病例标题 */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">病例标题 *</label>
              <input
                type="text"
                required
            ...
            {/* 病例描述 */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">病例描述</label>
              <textarea
            ...
            {/* 诊断 */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">诊断 *</label>
              <input
                type="text"
                required
            ...
            {/* 治疗方案 */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">治疗方案</label>
              <textarea
            ...
            {/* 治疗效果 */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">治疗效果</label>
              <textarea
            ...
            {/* 图片上传 */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">病例图片</label>
              <input
                type="file"
                accept="image/*"
                multiple
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {formData.imageUrls.map((url, idx) => (
                    <div key={idx} className="relative">
                      <img src={url} alt={`病例图片 ${idx + 1}`} className="w-20 h-20 object-cover rounded-lg" />
                      <button
                        type="button"
            ...
            <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
              ...
              <button
                ...
                disabled={isSubmitting}
              >
                {isSubmitting ? (isEditMode ? "更新中..." : "发布中...") : isEditMode ? "更新医案" : "发布医案"}
              </button>
            </div>
          </form>
        </div>
